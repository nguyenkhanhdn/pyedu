...
// Initialize SQLite database eagerly on startup
getDatabase().catch(err => console.error("SQLite init error:", err));
...
app.post("/api/ai/tutor", async (req, res) => {
  try {
    ...
    if (!ai) {
      // Fallback smart responses if no API key is provided
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`PyEdu server running on port ${PORT}`);
  });
}
